 ...
 * The loop reads every live value through [config]'s lambdas, so it can
 * outlive recomposition without ever restarting.
 */
internal suspend fun PointerInputScope.awaitGestures(config: EditorGestureConfig) {
        ...
        while (true) {
            ...
            if (config.isScaling()) {
                if (active.size >= 2) {
                    ...
                    if (factor != 1f && factor.isFinite()) {
                        ...
                        val newSize = (config.fontSizeSp() * factor)
                            .coerceIn(
                                EditorDimensions.MIN_FONT_SIZE_SP,
                                EditorDimensions.MAX_FONT_SIZE_SP,
                            )
                        val effective = newSize / config.fontSizeSp()
                        if (effective != 1f) {
            ...
            if (down != null) {
                ...
                if (gestureStart == null) {
                    ...
                    if (travelled.getDistance() > config.viewConfiguration.touchSlop) {
            ...
            if (released != null) {
                if (panning) {
                    ...
                    // back to letters.
                    if (!config.isImeVisible()) {
                        config.onReShowKeyboard()
                    }
                    config.onScrollTick()
                }
                gestureStart = null
