//! The [`CursorPosition`] type — a (row, column) byte pair the buffer,
//! cursor, input, and scroll camera all share.
...
/// pulled into its own type here because most call sites pass a position
/// around (scrolling, the Compose renderer's caret and tap lookups, and
/// the cursor arrow keys all need one).
